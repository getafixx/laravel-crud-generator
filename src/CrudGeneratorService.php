...
class CrudGeneratorService 
{
    ...
    protected function getTypeFromDBType($dbtype) {
        ...
        if(Str::of($dbtype)->contains('varchar') ) { return 'text'; }
        if(Str::of($dbtype)->contains('int') || Str::of($dbtype)->contains('float') ) { return 'number'; }
        if(Str::of($dbtype)->contains('date') ) { return 'date'; }
        return 'unknown';
    }
    ...
    protected function appendToEndOfFile($path, $text, $remove_last_chars = 0, $dont_add_if_exist = false) {
        ...
        if(!Str::of($content)->contains($text) || !$dont_add_if_exist) {
